<template>
  <div class="space-y-8">
    ...
    >
      <div>
        <h1
          class="text-2xl md:text-3xl font-black tracking-tight text-slate-900 dark:text-white"
        >
          Habits
        </h1>
        <p class="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">
          Build consistency and track streaks.
        </p>
      </div>
      <button
    ...
    <!-- Stats Bar -->
    <div
      class="flex items-center space-x-3 overflow-x-auto pb-2 no-scrollbar mb-4"
    >
      <div
        class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 px-4 py-2 rounded-xl flex items-center space-x-2 whitespace-nowrap shadow-sm"
      >
      ...
      >
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4 flex-1 min-w-0">
            <div
              class="w-14 h-14 bg-rose-50 dark:bg-rose-900/20 text-rose-500 dark:text-rose-400 rounded-2xl flex items-center justify-center text-2xl shrink-0"
            >
            ...
            <div class="min-w-0">
              <div class="flex items-center gap-2">
                ...
                <span
                  v-if="habit.phase"
                  class="text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full border border-opacity-50"
                  :class="[
                    habit.phase === 'identity'
              ...
              <p
                class="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-[0.2em] mt-1"
              >
                {{ habit.frequency }}
                <span v-if="habit.scheduled_days" class="ml-1 text-rose-400"
                  >· {{ formatScheduledDays(habit.scheduled_days) }}</span
                >
              </p>
            </div>
          </div>
        ...
        <div class="bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-5">
          ...
          <div class="flex justify-between items-center px-1">
            ...
            >
              ...
              >
                <svg
                  v-if="isHabitCompleted(habit, day.date)"
                  class="w-5 h-5"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
        ...
        <div class="flex items-center justify-between gap-4 mt-auto">
          <button
            ...
            @click="
              ...
              ) && toggleHabit(habit.id, new Date().toISOString().split('T')[0])
            "
          ...
          <div
            v-else
            class="flex-1 py-3.5 px-4 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-dashed border-slate-200 dark:border-slate-700 text-center flex items-center justify-center"
          >
            <span
              class="text-[10px] font-bold text-slate-400 uppercase tracking-widest"
              >Not scheduled for today</span
            >
          </div>
    ...
    >
      ...
      >
        ...
        <div
          class="p-4 md:p-6 space-y-4 md:space-y-6 overflow-y-auto custom-scrollbar"
        >
          <div>
            <label
          ...
          <div>
            <label
              class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2"
              >Atomic Habit Phase</label
            >
            <div class="grid grid-cols-3 gap-2">
              <button
                @click="newHabit.phase = 'identity'"
                class="py-2.5 rounded-xl text-xs font-bold transition-all border"
                :class="
                  newHabit.phase === 'identity'
                    ? 'bg-indigo-500 text-white border-indigo-500 shadow-lg shadow-indigo-200 dark:shadow-none'
              ...
              <button
                @click="newHabit.phase = 'process'"
                class="py-2.5 rounded-xl text-xs font-bold transition-all border"
                :class="
                  newHabit.phase === 'process'
                    ? 'bg-cyan-500 text-white border-cyan-500 shadow-lg shadow-cyan-200 dark:shadow-none'
                    : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700 text-slate-500 hover:border-cyan-200'
                "
              >
                Process
              </button>
              <button
                @click="newHabit.phase = 'outcome'"
                class="py-2.5 rounded-xl text-xs font-bold transition-all border"
                :class="
                  newHabit.phase === 'outcome'
                    ? 'bg-orange-500 text-white border-orange-500 shadow-lg shadow-orange-200 dark:shadow-none'
                    : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700 text-slate-500 hover:border-orange-200'
                "
              >
                Outcome
              </button>
            ...
            <p class="text-[10px] text-slate-400 mt-2 font-medium">
              <span v-if="newHabit.phase === 'identity'"
                >Who you wish to become (e.g. "I am a runner")</span
              >
              <span v-else-if="newHabit.phase === 'process'"
                >What you do (e.g. "Run 2 miles every day")</span
              >
              <span v-else>What you get (e.g. "Lose 10 pounds")</span>
            </p>
          </div>
          <div>
            <label
              class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2"
              >Link Goal (Optional)</label
            >
            <select
          ...
          <div v-if="newHabit.frequency === 'weekly'" class="space-y-6">
            ...
            <div class="space-y-3">
              ...
              >
              <div
                v-for="(task, idx) in newTaskItems"
                :key="idx"
              ...
              >
                ...
                />
                <button
                  @click="newTaskItems.splice(idx, 1)"
        ...
        <div
          class="p-4 md:p-6 bg-slate-50 dark:bg-slate-800/50 flex justify-end space-x-3 shrink-0"
        >
          <button
            @click="showNewHabitModal = false"
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <div class="grid grid-cols-3 gap-2">
              <button
                @click="editHabitData.phase = 'identity'"
                class="py-2.5 rounded-xl text-xs font-bold transition-all border"
                :class="
                  editHabitData.phase === 'identity'
                    ? 'bg-indigo-500 text-white border-indigo-500 shadow-lg shadow-indigo-200 dark:shadow-none'
                    : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700 text-slate-500 hover:border-indigo-200'
                "
              >
              ...
              <button
                @click="editHabitData.phase = 'process'"
                class="py-2.5 rounded-xl text-xs font-bold transition-all border"
                :class="
                  editHabitData.phase === 'process'
                    ? 'bg-cyan-500 text-white border-cyan-500 shadow-lg shadow-cyan-200 dark:shadow-none'
                    : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700 text-slate-500 hover:border-cyan-200'
                "
              >
              ...
              <button
                @click="editHabitData.phase = 'outcome'"
                class="py-2.5 rounded-xl text-xs font-bold transition-all border"
                :class="
                  editHabitData.phase === 'outcome'
                    ? 'bg-orange-500 text-white border-orange-500 shadow-lg shadow-orange-200 dark:shadow-none'
                    : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700 text-slate-500 hover:border-orange-200'
                "
              >
          ...
          <div v-if="editHabitData.frequency === 'weekly'" class="space-y-6">
            <div>
              ...
              <div class="flex justify-between gap-1">
                <button
                  ...
                  class="flex-1 h-10 rounded-xl font-bold text-xs transition-all border-2"
                  :class="
                    isDaySelectedInEditHabit(index)
                      ? 'bg-rose-500 border-rose-500 text-white shadow-lg shadow-rose-100'
                      : 'border-slate-100 dark:border-slate-800 text-slate-400 hover:border-rose-200'
                  "
                >
            ...
            <div class="space-y-3">
              ...
              >
              <div
                v-for="(task, idx) in editTaskItems"
                :key="idx"
                class="flex items-center gap-2"
              >
                <input
                ...
                />
                <button
                  @click="editTaskItems.splice(idx, 1)"
                  class="p-2 text-slate-400 hover:text-rose-500"
                >
                  <svg
                    class="w-4 h-4"
...
function toggleDayInNewHabit(dayIndex) {
  let days = newHabit.value.scheduled_days
    ? newHabit.value.scheduled_days.split(",")
    : [];
...
function toggleDayInEditHabit(dayIndex) {
  let days = editHabitData.value.scheduled_days
    ? editHabitData.value.scheduled_days.split(",")
    : [];
  const index = days.indexOf(dayIndex.toString());
  if (index === -1) {
    days.push(dayIndex.toString());
...
function isDaySelectedInEditHabit(dayIndex) {
  ...
  return editHabitData.value.scheduled_days
    .split(",")
    .includes(dayIndex.toString());
}
...
function calculateWeeklyCompletion(habit) {
  const completedCount = last7Days.value.filter((day) =>
    isHabitCompleted(habit, day.date),
  ).length;
